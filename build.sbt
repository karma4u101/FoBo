...
lazy val fobometa = (project in file(".")).
  ...
  settings(liftEdition in ThisBuild <<= liftVersion apply { _.substring(0,3) }).
  aggregate(fobo)
  
lazy val fobo = (project in file("FoBo/FoBo")).
  settings(commonSettings: _*).
  settings(name := "fobo"). 
  aggregate(foboapi,kineticjs,pace,angularjs,jquery,bootstrap,bootstrap3,fontAwesome,prettify,highlightjs).
...
lazy val bootstrap = (project in file ("Bootstrap/Bootstrap2/Twitter-Bootstrap")).
  settings(commonSettings: _*).  
  settings(name := "fobo-twitter-bootstrap").
  aggregate(bootstrapapi,bootstrapres).
  dependsOn(bootstrapapi,bootstrapres)

lazy val bootstrapapi = (project in file ("Bootstrap/Bootstrap2/Twitter-Bootstrap-API")).
  settings(commonSettings: _*).  
  settings(name := "fobo-twitter-bootstrap-api")

lazy val bootstrapres = (project in file ("Bootstrap/Bootstrap2/Twitter-Bootstrap-Res")).
  settings(commonSettings: _*).  
...
lazy val bootstrap3api = (project in file ("Bootstrap/Bootstrap3/TwBs-Bootstrap3-API")).
  settings(commonSettings: _*).  
  settings(name := "fobo-twbs-bootstrap3-api")

lazy val bootstrap3res = (project in file ("Bootstrap/Bootstrap3/TwBs-Bootstrap3-Res")).
  settings(commonSettings: _*).  
...
lazy val prettify  = (project in file ("Google-Code-Prettify/Google-Code-Prettify")).
  settings(commonSettings: _*).  
  settings(name := "fobo-google-code-prettify").
  aggregate(prettifyres).
  dependsOn(prettifyres)

lazy val prettifyres = (project in file ("Google-Code-Prettify/Google-Code-Prettify-Res")).
  settings(commonSettings: _*).  
...
//##
//##############
libraryDependencies in ThisBuild <++= (liftVersion,liftEdition,version) { (v,e,mv) =>
