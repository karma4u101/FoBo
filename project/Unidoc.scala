...
object Unidoc {
  ...
  lazy val GenJavaDocEnabled = Option(sys.props("genjavadoc.enabled")) filter (_.toLowerCase == "true") map (_ => true) getOrElse false
  ...
  lazy val javadocSettings =
    inConfig(JavaDoc)(Defaults.configSettings) ++ Seq(
      ...
      sources in JavaDoc <<= (target, compile in Compile, sources in Compile) map ((t, c, s) =>
        ...
        else throw new RuntimeException("cannot build java docs without -Dgenjavadoc.enabled=true")
      ),
