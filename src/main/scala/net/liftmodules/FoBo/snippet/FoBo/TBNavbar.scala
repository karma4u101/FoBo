...
object TBNavbar extends FlexMenuBuilder with DispatchSnippet {
  ...
  private def renderPullLeft : NodeSeq = {
    pullRight=false
    render
  }
  private def renderPullRight : NodeSeq = {
    pullRight=true
    render
  }   
  ...
  private def renderItemWithInfo(info: List[net.liftweb.common.Box[Function0[_]]], renderInner: Seq[MenuItem] => NodeSeq, item: MenuItem): Elem = {
    ...
    def buildWithInfo(f: Function0[_], renderInner: Seq[MenuItem] => NodeSeq, item: MenuItem): Elem = {
      if (f().equals("divider")) {
        ...
        buildInnerTag(<xml:group><li class="divider-vertical"></li></xml:group>, item.path, item.current)
      } else {
